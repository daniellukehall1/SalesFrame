...
- Browser WebSocket auth must use Deepgram's documented `Sec-WebSocket-Protocol` pattern: `token, <temporary credential>`. Do not try `bearer`/`Bearer` WebSocket subprotocol variants before `token`; they can consume the 30-second temporary token window before the valid browser handshake is attempted.
