...
test("account contacts use the shared responsive record-management UI", async () => {
  ...
  assert.match(contacts, /className="grid grid-cols-2 gap-2 xl:grid-cols-\[minmax\(0,1fr\)_180px_190px\]"/)
  assert.match(contacts, /className="relative col-span-2 xl:col-span-1"/)
  ...
  assert.match(contacts, /isMobile \? \([\s\S]*<DrawerFooter className="-mx-3 mt-2 border-t/)
  assert.match(contacts, /function ContactEnrichmentStatusText/)
  assert.doesNotMatch(contacts, /function EnrichmentBadge/)
  assert.match(accountRecordTab, /Archive account/)
  assert.doesNotMatch(accountContactsTab, /Archive account/)
...
test("contact enrichment distinguishes queued work from a true empty state", async () => {
  ...
  const enrichmentDetails = contacts.slice(
    ...
    contacts.indexOf("function ContactEnrichmentStatusText")
  )
...
test("contact identities open a complete responsive canonical record", async () => {
  ...
  assert.match(detailOverlay, /<dl className="grid min-w-0 gap-3 md:grid-cols-2 xl:grid-cols-4">/)
  assert.match(detailOverlay, /overflow-x-hidden overflow-y-auto overscroll-contain/)
  assert.match(detailOverlay, /\[scrollbar-gutter:stable\]/)
  assert.match(detailOverlay, /overflow-x-hidden overflow-y-hidden/)
  assert.match(detailOverlay, /sm:w-\[calc\(100%_-_3rem\)\] sm:max-w-6xl/)
  ...
  assert.match(detailOverlay, /contact\.archivedAtIso \? <ContactDetailField label="Archive status"/)
  assert.doesNotMatch(detailOverlay, /"Active record"/)
  ...
  assert.match(detailOverlay, /\[contact\.jobTitle, contact\.department\]\.filter\(Boolean\)\.join\(" · "\)/)
  assert.doesNotMatch(detailOverlay, /<EnrichmentBadge/)
  assert.match(contacts, /grid-cols-1 items-start gap-3 lg:grid-cols-2/)
  assert.doesNotMatch(contacts, /repeat\(auto-fit/)
  assert.match(contacts, /w-full min-w-0 max-w-full overflow-hidden rounded-md bg-background\/70/)
  assert.match(contacts, /min-w-0 flex-1 whitespace-normal break-words \[overflow-wrap:anywhere\]/)
...
test("contact directories keep enrichment state calm and actions deliberate", async () => {
  const contacts = await read("src/components/contact-management.tsx")
  const mobileDirectory = contacts.slice(
    contacts.indexOf('data-testid="account-contacts-mobile-list"'),
    contacts.indexOf('data-testid="account-contacts-table"')
  )

  assert.doesNotMatch(mobileDirectory, /<Badge/)
  assert.doesNotMatch(mobileDirectory, /Actions for \$\{contact\.fullName\}/)
  assert.doesNotMatch(mobileDirectory, /Created \{contact\.createdAt\}/)
  assert.match(contacts, /status === "completed"[\s\S]*\? "Ready"/)
  assert.match(contacts, /status === "queued" \|\| status === "running"[\s\S]*\? "Updating…"/)
  assert.match(contacts, /disabled=\{!canEnrich \|\| pendingContactId === contact\.id \|\| \["queued", "running"\]\.includes\(getEnrichmentStatus\(contact\)\)\}/)
  assert.match(contacts, /\["queued", "running"\]\.includes\(getEnrichmentStatus\(contact\)\)[\s\S]*"Enrichment in progress"/)
})
...
test("contact management keeps search, forms, filters, and async saves accessible", async () => {
  ...
  assert.match(contacts, /<SelectItem value="unknown">Unknown<\/SelectItem>/)
