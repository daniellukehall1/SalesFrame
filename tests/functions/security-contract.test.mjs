...
test("OpenAI helper supports strict structured output schemas", async () => {
  ...
  assert.match(openai, /OPENAI_REALTIME_TRANSCRIPTION_MODEL/)
  assert.match(openai, /supportedRealtimeTranscriptionModels/)
  assert.match(openai, /getRealtimeTranscriptionModel/)
  assert.match(openai, /const legacyTranscriptionModel = getEnv\("OPENAI_TRANSCRIPTION_MODEL"\)/)
  assert.match(openai, /return "gpt-realtime-whisper"/)
