...
test("expensive AI functions enforce authenticated rate limits", async () => {
  ...
  assert.match(liveState, /authorizeAccount\(user\.id, payload\.accountId, supabase, \{ token \}\)/)
  assert.match(liveState, /authorizeOpportunity\(user\.id, payload\.opportunityId, supabase, \{ token \}\)/)
  assert.match(liveState, /assertCallIsLive\(call\)/)
