  ...
  try {
    ...
    const rawGuidanceResult = hasTranscript
      ? await callOpenAiJson<unknown>({
        ...
        input: JSON.stringify({
          ...
          coachingRules: [
            ...
            "Next Call Preparation is not live question history and cannot establish evidence, completion, or a reason to replace the current card.",
      ...
      : hydratePreCallGuidanceResult(await callOpenAiJson<PreCallLiveGuidanceResult>({
        ...
        system:
          contactCoachingSystemInstruction + "\n\n" +
          "You are SalesFrame's live enterprise sales coach. This request is active-call opening guidance: no customer transcript exists yet. It is separate from the seller-facing Next Call Preparation artifact. Return one natural first live question that helps the seller open the call calmly and advances the selected playbooks without sounding like a checklist. Preparation content is not a queued live card, question history, or buyer evidence; never copy or reconstruct its exact wording, and never treat it as asked, answered, or complete. Rank exactly three internal candidate moves using the provided intentClusters and recordContext. The selected playbooks and intent clusters decide what needs to be learned; account, opportunity, customer research, seller research, and enrichment context only shape timing and wording. Do not ask heavy budget, procurement, economic-buyer, champion, quantified-metrics, or hard decision-process questions in the opener unless the provided opportunity history clearly says the buyer already raised that topic. If Sandler is selected, prefer a natural upfront-contract or ANOT-style opener. Return only schema-valid JSON.",
        input: JSON.stringify({
          selectedContext: {
            callType: selectedCallType,
            selectedContacts: selectedContactContext,
          ...
          coachingRules: [
            ...
            "Keep Next Call Preparation separate: it is not live question history, evidence, or exact wording for this card.",
